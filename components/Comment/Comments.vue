    ...
    <div>
        <div v-if="!comments || comments?.length === 0">
            <div class="flex flex-col justify-center items-center m-4 pb-6 space-y-4">
                <UIcon name="fluent:comment-lightning-24-regular" class="w-12 h-12 text-gray-800" />
                <div class="text-sm">欢迎在评论区分享您的想法！</div>
            </div>
        </div>
        <div v-else-if="status === 'pending'">
            <div class="flex flex-col justify-center items-center m-4 pb-6 space-y-2.5">
                <UIcon name="ri:loader-3-line" class="w-8 h-8 animate-spin" />
        ...
        <div class="space-y-5 pb-10" v-else>
            ...
            <div v-for="(comment, index) in recentComments" :key="comment.id" class="relative my-6">
                    ...
                    <div class="group flex items-center">
                        ...
                        <div class="ml-4 -space-y-0.5">
                            ...
                            <div class="text-[0.8rem] text-gray-600">
                                {{ useFormatDate(comment.date_created) }}
                            </div>
                        </div>
                    </div>
                ...
                <div class="ml-14">
                    <MarkdownRenderer :markdown="comment.comment" :enableHtml="false" class="text-gray-300" />
                </div>
                    ...
                    <div v-for="(reply, replyIndex) in comment.replies.slice(0, isExpanded[comment.id] ? comment.replies.length : 2)"
                        ...
                        <div class="flex flex-row justify-between items-center" :ref="setReplyRef(reply.id)">
                            <div class="group flex items-center">
                                ...
                                <div class="ml-4 -space-y-0.5">
                                    <div class="text-base font-medium text-gray-200 flex items-center space-x-3">
                                        <div>{{ reply.user_created.first_name }}</div>
                                    </div>
                                    <div class="text-[0.8rem] text-gray-600">
                                        {{ useFormatDate(reply.date_created) }}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="ml-14">
                            <MarkdownRenderer :markdown="reply.comment" :enableHtml="false" class="text-gray-300" />
                        </div>
                    </div>
                    ...
                    <div v-if="comment.replies.length > 2" class="ml-14 mt-4">
                        <button type="button" @click="toggleExpand(comment.id)"
                            ...
                            <span v-if="isExpanded[comment.id]" class="flex items-center">
                                收起回复
                                <UIcon name="ri:arrow-up-s-line" class="w-5 h-5 ml-1" />
                            </span>
                            <span v-else class="flex items-center">
                                展开 {{ comment.replies.length - 2 }} 条回复
                                <UIcon name="ri:arrow-down-s-line" class="w-5 h-5 ml-1" />
                            </span>
                        </button>
                    </div>
                </div>
            ...
            <div v-if="showOldCommentsButton && oldComments.length > 0" class="flex justify-center pt-8">
                <UBadge :ui="{ rounded: 'rounded-lg' }" label="已折叠部分评论" size="md" variant="soft" color="gray"
                    ...
                    <template #trailing>
                        <UIcon name="ri:arrow-down-s-line" class="w-5 h-5" />
                    </template>
                </UBadge>
            </div>
        ...
        <UModal v-model="isDeleteModalOpen" prevent-close :transition="false">
            <div class="p-6">
                ...
                <div class="flex justify-end space-x-3">
                    <UButton label="取消" color="gray" @click="isDeleteModalOpen = false" />
                    <UButton label="确认删除" color="red" :loading="isDeleting" :disabled="isDeleting"
                        loading-icon="ri:loader-3-line" @click="confirmDelete" />
                </div>
            </div>
        </UModal>
    </div>
